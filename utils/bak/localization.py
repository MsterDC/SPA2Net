...
from utils.bak.vistools import norm_atten_map
...
def get_box_sos(pred_scm, im_file, threshold, gt_labels):
    ...
    sc_map = cv2.resize(pred_scm, dsize=(w, h))  # (14,14) => bak size
...
def get_topk_boxes_scg_v2(logits, top_cams, sc_maps, im_file, topk=(1,), gt_labels=None, threshold=0.2,
    ...
    for i in range(maxk):
        ...
        for j, sc_map in enumerate(sc_maps):
            ...
            cam_map_cls = top_cams[i]  # The size of CAM is the same as bak image.
            ...
            sc_map_cls_i = cv2.resize(sc_map_cls_i, dsize=(w, h))  # (14,14) => bak image size
