...
def debug_vis_loc(args, idx, show_idxs, img_path, top_maps, top5_boxes, label, gt_boxes, suffix='cam'):
    if not args.debug:
        return
    ...
    for vis_n in range(base_num):
        if idx == vis_n:
            for show_id in show_idxs:
                save_im_heatmap_box(img_path[show_id], top_maps[show_id], top5_boxes[show_id], args.debug_dir,
...
def debug_vis_sc(args, idx, show_idxs, img_path, sc_maps_fo_fuse, sc_maps_so_fuse, sc_maps_fuse, label, suffix=None):
    if not args.debug:
        return
    ...
    for vis_n in range(base_num):
        if idx == vis_n:
            for show_id in show_idxs:
                save_im_sim(img_path[show_id], sc_maps_fo_fuse[show_id], args.debug_dir, gt_label=label[show_id], epoch=args.current_epoch, suffix=suffix[0])
                save_im_sim(img_path[show_id], sc_maps_so_fuse[show_id], args.debug_dir, gt_label=label[show_id], epoch=args.current_epoch, suffix=suffix[1])
                save_im_sim(img_path[show_id], sc_maps_fuse[show_id], args.debug_dir, gt_label=label[show_id], epoch=args.current_epoch, suffix=suffix[-1])
        pass
