...
def save_im_heatmap_box(im_file, top_maps, topk_boxes, save_dir, gt_label=None, gt_box=None,
    ...
    for cls_box, cam_map_cls in zip(topk_boxes, top_maps):
        ...
        # @modified by Kevin
        # if suffix == 'sos' or suffix == 'sst':
        #     ratio_color = 700
        #     gray_map = ratio_color * cam_map_cls
        #     # cv2.normalize(gray_map, gray_map, 0, 255, cv2.NORM_MINMAX)
        ...
        #
        #     t_hm[gray_map < 128] = 0
        #     t_hm[gray_map >= 128] = 255
        #     gray_map[gray_map < 128] = 0
        #     gray_map[gray_map >= 128] = 255
        #
        #     hname = str(gt_label[0]) + '_' + str(epoch) + '_' + im_file.split('/')[-1]
        #     hname = hname.replace('.', 'th-{}_{}.'.format(threshold, 'hmp'))
        #     cv2.imwrite(os.path.join(save_dir, hname), t_hm)
        #
        #     gray_map = gray_map.astype(np.float32) / 255
        #     gray_map = torch.from_numpy(gray_map)
        #     img = torch.from_numpy(np.transpose(im, (2, 0, 1)).astype(np.float32)/255)
        #     masked = gray_map.cuda() * img.cuda()  # (bs,c,w,h)
        #     img_masked = masked.mul(255).byte()
        #     img_masked = img_masked.cpu().numpy().transpose((1, 2, 0))
        #     # masked = cv2.add(im, np.zeros(np.shape(im), dtype=np.uint8), mask=gray_map)
        #     masked_name = hname.replace('.', 'th-{}_{}.'.format(threshold, 'masked'))
        #     cv2.imwrite(os.path.join(save_dir, masked_name), img_masked)
        # test end  #
...
def save_sim_heatmap_box(im_file, top_maps, save_dir, gt_label=None, sim_map=None,
    ...
    # draw_im[:h, :, :] = im
    # cv2.putText(draw_im, 'original image: {}'.format(threshold), (0, h + 12),
