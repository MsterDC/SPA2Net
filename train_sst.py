...
def train(args):
    ...
    while current_epoch < total_epoch:
        ...
        for idx, dat in enumerate(train_loader):
            ...
            if 'hinge' in args.mode:
                hg_cls_logits = torch.mean(torch.mean(logits_hg, dim=2), dim=2)
                ...
                for idx, im in enumerate(img_path):
                    if want_im in im and save_flag is True:
                        ...
                        if 'sos' in args.mode and current_epoch >= args.sos_start:
                            ...
                            if args.sos_seg_method == 'BC' and 'mc_sos' not in args.mode:
        ...
        if args.watch_cam:
            ...
            if 'sos' in args.mode and current_epoch >= args.sos_start:
                ...
                if 'mc_sos' in args.mode:
                    watch_sos = watch_sos[0]
                    output = F.sigmoid(watch_sos[watch_label, ...])
                    watch_sos = [
                        torch.where(output >= args.sos_fg_th, torch.ones_like(output), torch.zeros_like(output))]
            ...
            if 'hinge' in args.mode:
                save_cam(args, watch_trans_img, watch_hg_cam, watch_hg_cls_logits, watch_img_path, watch_label, current_epoch, suffix='cam_hg')
