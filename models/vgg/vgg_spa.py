...
class VGG(nn.Module):
    ...
    # @ Writen by Chen Dong
    # def mst_hsc(self, f_phi, mst_th):
    #     """
    #     Generate MST from original feature map and calculate similarity.
    #     :param f_phi: feature of certain stage
    #     :param mst_th: mst threshold
    #     :return: mst sc
    #     """
    #     # (h, w) = (14, 14)
    #     # f_phi = F.interpolate(f_phi, size=(h, w), mode='bilinear', align_corners=True)
    #     n, c_nl, h, w = f_phi.size()
    #     base_th = 1. / (h * w)
    #     mst_th = base_th * mst_th
    #     mst_ho = mstTest.inter(f_phi)
    #     mst_ho = mst_ho.float()
    #     # mst_ho[mst_ho < mst_th] = 0
    #     return mst_ho
    #
    # def lsc(self, f_phi, fo_th=0.1):
    #     n, c_nl, h, w = f_phi.size()
    #     # (h, w) = (14, 14)
    #     # f_phi = F.interpolate(f_phi, size=(h, w), mode='bilinear', align_corners=True)
    #     f_phi = f_phi.permute(0, 2, 3, 1).contiguous().view(n, -1, c_nl)
    ...
    #     non_local_cos_fo = non_local_cos.clone()
    #     non_local_cos_fo = non_local_cos_fo / (torch.sum(non_local_cos_fo, dim=1, keepdim=True) + 1e-5)
    #     return non_local_cos_fo
    #
    # def sot(self, f_phi, fo_th=0.2, so_th=1):
    #     n, c_nl, h, w = f_phi.size()
    #     sot = sotTest.main(f_phi)
    #     sot = sot.float()
    #     base_th = 1. / (h * w)
    #     so_th = base_th * so_th
    #     sot[sot < so_th] = 0
    ...
    #     non_local_cos_fo = non_local_cos_fo / (torch.sum(non_local_cos_fo, dim=1, keepdim=True) + 1e-5)
    #
    #     return non_local_cos_fo, sot
    #
    # def max_hsc(self, f_phi, fo_th=0.2, so_th=0.2):
    #     n, c_nl, h, w = f_phi.size()
    #     if h != 14 or w != 14:
    #         h, w = 14, 14
    #         f_phi = F.interpolate(f_phi, size=(h, w), mode='bilinear', align_corners=True)
