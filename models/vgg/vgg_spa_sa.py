...
# from utils import mstTest, sotTest
...
class VGG(nn.Module):
    def __init__(self, features, num_classes=1000, cnvs=(10, 17, 24), args=None):
        ...
        self.thr_avg_pool = ThresholdedAvgPool2d(threshold=args.th_avgpool)
    ...
    def forward(self, x, train_flag=True, scg_flag=False, current_epoch=200):
        x = self.conv1_2(x)
        sc_2 = None
        sc_2_so = None
        if scg_flag and not train_flag and '2' in self.args.scg_blocks:
        ...
        sc_3 = None
        sc_3_so = None
        if scg_flag and not train_flag and '3' in self.args.scg_blocks:
        ...
        sc_4 = None
        sc_4_so = None
        if scg_flag and not train_flag and '4' in self.args.scg_blocks:
        ...
        sc_5 = None
        sc_5_so = None
        if scg_flag and not train_flag and '5' in self.args.scg_blocks:
            sc_5, sc_5_so = self.hsc(feat_5, fo_th=self.args.scg_fosc_th,
                                     so_th=self.args.scg_sosc_th,
                                     order=self.args.scg_order)

        if self.args.sa:  # spa+sa
            if train_flag:  # train
                if current_epoch >= self.args.sa_start:
                    sc_map_max = None
                    if self.args.sa_edge_encode == 'True':
                        sc_5, sc_5_so = self.hsc(feat_5, fo_th=0.2, so_th=1, order=2)
                        # sc_5 = sc_5.clone().detach()
                        # sc_5_so = sc_5_so.clone().detach()
                        sc_map_max = torch.max(sc_5, sc_5_so)
                        # sc_map_max = sc_map_max.clone().detach()
                    edge_code = sc_map_max if self.args.sa_edge_encode == 'True' else None
                    _n, _c, _, _ = feat_5.shape
                    sa_input = feat_5.view(_n, _c, -1).permute(0, 2, 1)  # n, h*w, c
                    cls_layer_in = self.sa(sa_input, sa_input, sa_input, edge_code=edge_code)
                else:
                    cls_layer_in = feat_5
                pass
            else:  # test
                if self.args.use_sa == 'True':  # test with sa module
                    sc_map_max = None
                    if self.args.sa_edge_encode == 'True':
                        sc_5, sc_5_so = self.hsc(feat_5, fo_th=self.args.scg_fosc_th,
                                                 so_th=self.args.scg_sosc_th,
    ...
    def get_loss(self, logits, gt_child_label, epoch=0, ram_start=10):
        if self.args.use_thr_pool == 'True' and self.args.thr_pool_start <= epoch:
            # threshold pooling
    ...
    def get_ra_loss(self, logits, label, th_bg=0.3, bg_fg_gap=0.0):
        n, _, _, _ = logits.size()
        cls_logits = F.softmax(logits, dim=1)  # along channel dim to cal
        var_logits = torch.var(cls_logits, dim=1)  # (n,c,w,h) -> (n,w,h)
        ...
        cls_map = logits[torch.arange(n), label.long(), ...]  # (n,w,h)
        cls_map = torch.sigmoid(cls_map)

        ra_loss = torch.mean(cls_map * bg_mask + (1 - cls_map) * fg_mask)  # ele-wise product
        return ra_loss
